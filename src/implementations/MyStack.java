...
public class MyStack<E> implements StackADT<E> {
    ...
    public E[] toArray(E[] copyArray) throws NullPointerException {
        if (copyArray == null) throw new NullPointerException("Array cannot be null");
        Object[] arr = list.toArray();
        if (copyArray.length < arr.length) {
            copyArray = (E[]) java.lang.reflect.Array.newInstance(copyArray.getClass().getComponentType(), arr.length);
        }
        for (int i = 0; i < arr.length; i++) {
            copyArray[i] = (E) arr[arr.length - 1 - i];
        }
        if (copyArray.length > arr.length) {
    ...
    private class StackIterator implements Iterator<E> {
        ...
        public StackIterator() {
            copy = toArray(); // Contains elements from top to bottom
        }
